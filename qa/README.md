...
| `oauth-setup` | The two `/api/booking/google/*` routes answer 404 unless `GOOGLE_OAUTH_SETUP_SECRET` is set, which is the state a deployment should normally be in. Six probes, including a guessed secret and a forged callback; also asserts no response is token-shaped and the callback is never cached. |
