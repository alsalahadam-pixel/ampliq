...
| `oauth-setup` | The three `/api/booking/google/*` routes exist and refuse: 503 while `GOOGLE_OAUTH_SETUP_SECRET` is unset, 401 when it is set but not presented. Seven probes, including a guessed secret and a forged callback. Also asserts no response is token-shaped, the pages are never cached, and the callback source contains no `console` call — the refresh token goes to storage, never to a log. |
